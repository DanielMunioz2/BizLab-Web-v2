<?php 
    ...
    $queryUserName = "SELECT user_nombre FROM usuarios WHERE user_correo = '$userCorreo'";
    ...
    if ($resultUserName && $resultUserName->num_rows > 0) {
        ...
        $userName = $userNameData['user_nombre'];
    } else {
    ...
    $searchQuery = $search ? "AND user_nombre LIKE '%$search%'" : '';

    if ($filter == 'administradores') {
        $query = "SELECT id_usuario, user_nombre, user_rol, user_fechaU, user_fechaS, user_horaS, user_cargo, m.membre_nombre, user_estado
            FROM usuarios LEFT JOIN membresias m ON user_membresia = m.id_membresia WHERE user_rol = 'Administrador' $searchQuery LIMIT $limit OFFSET $offset";
    } else { 
        $query = "SELECT id_usuario, user_nombre, user_rol, user_fechaU, user_fechaS, user_horaS, user_cargo, m.membre_nombre, user_estado
            FROM usuarios LEFT JOIN membresias m ON user_membresia = m.id_membresia WHERE user_rol = 'Miembro' $searchQuery LIMIT $limit OFFSET $offset";
    }
    ...
    if ($filter == 'administradores') {
        $countQuery = "SELECT COUNT(*) as total FROM usuarios WHERE user_rol = 'Administrador' $searchQuery";
    } else { 
        $countQuery = "SELECT COUNT(*) as total FROM usuarios WHERE user_rol = 'Miembro' $searchQuery";
    }
...
<!DOCTYPE html>
<html lang="es" id="listaMiembrosHTML">
    <head>
        <meta charset="UTF-8">
        ...
        <main class="main">
            <div class="encabezado">
                ...
                <div class="navegacion">
                    <a href="?filter=miembros&page=1" class="navegar <?php echo ($filter == 'miembros') ? 'active' : ''; ?>">Miembros</a>
                    <a href="?filter=administradores&page=1" class="navegar <?php echo ($filter == 'administradores') ? 'active' : ''; ?>">Administradores</a>
                    <form class="search-form" method="GET" action="">
                        <input type="hidden" name="filter" value="<?php echo $filter; ?>">
                        <input type="text" name="search" class="search-input" placeholder="Buscar usuario..." value="<?php echo $search; ?>">
            ...
            <div class="contenedor_todo">
                <div class="listado">
                    <div class="listado-encabezado">
                        ...
                        <div class="listado-item2">Rol del Usuario</div>
                        <div class="listado-item2">Membresía del Usuario</div>
                        <div class="listado-item2">Cargo del Usuario</div>
                        <div class="listado-item2">Fecha de Unión</div>
                        <div class="listado-item2">Estado del Usuario</div>
                    </div>
                    <?php
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $rol = $row['user_rol'];
                                $membresia = $row['membre_nombre'];
                                $fechaUnion = $row['user_fechaU'];
                                $cargo = $row['user_cargo'];
                                $estado = $row['user_estado'];
                                
                                echo "<a href='informacionUsuario.php?id_usuario=$id_usuario'><div class='listado-row'>
                                    <div class='listado-item'>$nombre</div>
                                    <div class='listado-item'>$rol</div>
                                    <div class='listado-item'>$membresia</div>
                                    <div class='listado-item'>$cargo</div>
                                    <div class='listado-item'>$fechaUnion</div>
                                    <div class='listado-item' id='estado-$id_usuario'>$estado</div>
                                </div></a>";
                            }
                        } else {
                            echo "<div class='listado-row'><div class='listado-item' colspan='7'>No hay usuarios registrados.</div></div>";
                        }
                    ?>
                </div>
                <div class="pagination">
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <a href="?filter=<?php echo $filter; ?>&page=<?php echo $i; ?>&search=<?php echo $search; ?>" class="<?php if ($i == $page) echo 'active'; ?>"><?php echo $i; ?></a>
                    <?php endfor; ?>
                </div>
            </div>
        </main>
