...
if ($result && $result->num_rows > 0) {
    ...
    $producto_query = "SELECT produNombre FROM productos WHERE id_producto = '$id_producto'";
    $producto_result = $conn->query($producto_query);
    $producto = $producto_result->fetch_assoc();

    $id_membresia = $factura['id_membresia'];
    $membresia_query = "SELECT membre_nombre FROM membresias WHERE id_membresia = '$id_membresia'";
    $membresia_result = $conn->query($membresia_query);
    $membresia = $membresia_result->fetch_assoc();

    $id_usuario = $factura['id_usuario'];
    $usuario_query = "SELECT user_nombre FROM usuarios WHERE id_usuario = '$id_usuario'";
...
<html lang="es">
<head>
    <title>Información de la Factura</title>
    <meta charset="UTF-8">
...
<header class="header">
            <div class="header_div1">
                <div class="divPerfilContaint">
                    ...
                    <div class="divNombreCont">
                        <span class="nombrePerfil"><?php echo $userName; ?></span>
                    ...
                    <div class="cuadroOPerfil1 cuadroPOculto">
                        <div class="div1">
                            <div>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 312.57 425.95"><g id="Capa_2" data-name="Capa 2"><g id="Capa_1-2" data-name="Capa 1"><path d="M312.5,361.84a41.53,41.53,0,0,1-6.35,19.76,39.76,39.76,0,0,1-6.51,7.89,50.8,50.8,0,0,1-7.36,5.54c-2.73,1.72-5.46,3.44-8.29,5-3.08,1.69-6.18,3.36-9.37,4.86-4.13,1.95-8.33,3.73-12.58,5.38-5.46,2.11-11,3.92-16.67,5.5-6.64,1.86-13.36,3.38-20.14,4.64-4.1.76-8.21,1.45-12.34,2-2.8.36-5.6.76-8.4,1.08-2.36.28-4.74.47-7.1.69-1.83.17-3.65.37-5.48.51s-3.95.25-5.93.37l-9,.52-.75,0c-6.64,0-13.28.53-19.23.31-3.13,0-5.56,0-8,0-4.21,0-8.42-.25-12.64-.35-2.07-.05-4.16-.07-6.23-.28s-4.36-.16-6.53-.44c-1.81-.23-3.65-.25-5.48-.39-1.68-.13-3.36-.3-5-.47-1.24-.12-2.47-.24-3.7-.39-3.54-.42-7.09-.8-10.62-1.3q-7.51-1.06-15-2.56A225.53,225.53,0,0,1,61.44,414q-7.19-2.25-14.15-5A164.38,164.38,0,0,1,31,401.35a146.86,146.86,0,0,1-13.1-8A39.53,39.53,0,0,1,5.54,380.12,41.2,41.2,0,0,1,.75,367.05a26.61,26.61,0,0,1-.51-4.28,12,12,0,0,0-.14-1.33,14.79,14.79,0,0,1,0-3.12c.14-1.67.3-3.35.49-5,.32-2.76.62-5.52,1-8.27.64-4.62,1.51-9.19,2.5-13.75,1-4.75,2.27-9.45,3.65-14.11a164.4,164.4,0,0,1,6-16.67q1.52-3.63,3.2-7.2a161.83,161.83,0,0,1,9.27-17,150.11,150.11,0,0,1,9.06-12.9c2.29-2.93,4.73-5.74,7.15-8.56.23-.26.48-.5.72-.75,1.19-1.26,2.43-2.48,3.56-3.8a18.63,18.63,0,0,1,2.68-2.57,10.13,10.13,0,0,0,.77-.7,135.1,135.1,0,0,1,14.5-12.31A150.18,150.18,0,0,1,81.3,223.93a162,162,0,0,1,19.2-9.12,159.43,159.43,0,0,1,23.61-7.17c4.8-1.07,9.64-1.86,14.51-2.48,2.65-.34,5.32-.51,8-.73,5.2-.43,10.41-.
.62,32.62,0,0,1-8.42.7q-74.34,0-148.69,0a42.94,42.94,0,0,1-7.44-.49c-7.82-1.41-12.61-7.31-12.43-15.05A14.76,14.76,0,0,1,112.3,156.4a38.33,38.33,0,0,1,6-.26Q156,156.11,193.61,156.13Z"/><path d="M193.6,240.08q-37.17,0-74.34,0a34.38,34.38,0,0,1-9.39-1,14.31,14.31,0,0,1-10.46-14.63,14.12,14.12,0,0,1,11-13.63,36.19,36.19,0,0,1,7.91-.81q75.33-.07,150.67,0a35.65,35.65,0,0,1,6.92.64,14.42,14.42,0,0,1,12,13.9,14.66,14.66,0,0,1-11.09,14.74,30.88,30.88,0,0,1-8.41.81Z"/><path d="M193.84,132.32q-36.42,0-72.83,0a61.85,61.85,0,0,1-9.43-.67,14,14,0,0,1-12.14-13.18c-.4-7.18,3.66-13.23,10.27-15.3a24,24,0,0,1,7.4-.9q76.56,0,153.14,0c11.07,0,17.72,5.85,17.62,15.26a14.26,14.26,0,0,1-11.29,14,45,45,0,0,1-8.91.78Q230.76,132.38,193.84,132.32Z"/></g></g></svg>
                                Soporte
                            </button></a>
                        </div>
                        <div class="div3">
    ...
    <main class="main">    
        <div class="contenedor_todo_usuario">
            <div class="contenedor_base_usuario">
                <div class="base1">
                    <div class="informacion_usuario">
                        <h3>Detalle de la información de la factura</h3>
                    </div>
                    <div class="contenedor_texto">
                        <div class="texto_base">
                            <p class="texto">Nombre usuario:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($usuario['user_nombre'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Referencia Epayco:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['refEpayco'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Factura Epayco:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['factuEpayco'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base"> 
                            <p class="texto">Código:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['facturaCodigo'] ?? 'No disponible'); ?></h3> 
                        </div>
                    </div>
                    <div class="contenedor_texto">
                        <div class="texto_base">
                            <p class="texto">Estado:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['estadoFactura'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Precio:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['precioFactura'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">IVA:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['ivaFactura'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Descuento:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['descuFactura'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Precio total:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['montoFactuTotal'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Oficina:</p>
                            <h3 class="texto"><?php?></h3>
                        </div>
                    </div>
                    <div class="contenedor_texto">
                        <div class="texto_base">
                            <p class="texto">Token Tarjeta:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['tokenTarjeta'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Banco:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($factura['bancoNombre'] ?? 'No disponible'); ?></h3> 
                        </div>
                        <div class="texto_base">
                            <p class="texto">Membresía:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($membresia['membre_nombre'] ?? 'No disponible'); ?></h3>
                        </div>
                        <div class="texto_base">
                            <p class="texto">Producto:</p>
                            <h3 class="texto"><?php echo htmlspecialchars($producto['produNombre'] ?? 'No disponible'); ?></h3>
                        </div>
                    </div>
                </div>
                <div class="base_2">
                    <div class="base3">
                        <div class="contenedor_usuario1">
                            <div class="imagen_usuario">
                                <img src="images/factura.png">
                            </div>
                        </div>
                        <div class="contenedor_usuario2">
                            <div class="descargar_pdf">
                                <a target="_blank" href="pdfFactura.php?id_factura=<?php echo $id_factura; ?>" class="boton2">Descargar información</a>
                            </div>
                        </div>
                        <div class="navegacion_usuario">
                            <button class="boton" onclick="history.back()"><< Regresar al listado de facturas</button>
                        </div>
                    </div>
                    <div class="ayuda">
                        <a target="_blank" href="https://wa.me/+"><h2>¿Cómo podemos ayudarte?</h2></a>
                    </div>
                </div>
            </div>
        </div>
